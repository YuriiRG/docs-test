...
export default function AddSubVectors() {
  ...
  return (
    <div>
      <h2 className="text-2xl">
        Онлайн калькулятор. Додавання та різниця двох векторів
      </h2>
      <div>
...
const vectorSchema = z.object({
  ...
  operation: z.string().pipe(z.enum(["plus", "minus"])),
});
...
function calculateResult(input: z.input<typeof vectorSchema>) {
  ...
  const { startPoint1, endPoint1, startPoint2, endPoint2, operation } =
    parseResult.data;

  const vector1 = convertPointsToVector(startPoint1, endPoint1);
  const vector2 = convertPointsToVector(startPoint2, endPoint2);

  return vector1.map((n, i) =>
    operation === "plus" ? n + vector2[i] : n - vector2[i],
  );
}
